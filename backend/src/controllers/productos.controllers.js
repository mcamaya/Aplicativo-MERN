...
export const getAllProductos = async (req, res) => {
  try {
    const productosdDB = await Producto.find({ activo: true }).populate(
      "proveedor"
    );
...
export const getProductByID = async (req, res) => {
  try {
    const productoDB = await Producto.findOne({ _id: req.params.id }).populate(
      "proveedor"
    );
...
export const postNewProduct = async (req, res) => {
  try {
    ...
    const newItemInventario = new Inventario({
...
export const deleteProducto = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ msg: "Producto marcado como inactivo", deletedProducto });
  } catch (err) {
    httpErrors(res, err);
  }
};
